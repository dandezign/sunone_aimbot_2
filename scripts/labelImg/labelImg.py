...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(
        ...
        auto_annotate = action(
            "Auto-annotate",
            self.auto_annotate_image,
            "space",
            "auto",
            "Automatically annotate the current image",
            enabled=True,
        )
    ...
    def load_models(self):
        ...
        if not os.path.exists(model_dir):
            QMessageBox.warning(
                self, "Warning", f"Dir with models '{model_dir}' not found."
            )
            return
    ...
    def auto_annotate_image(self):
        if not AI_AVAILABLE:
            QMessageBox.warning(
                self,
                "Warning",
                "AI features not available. Please install torch and ultralytics.",
            )
            return
        ...
        results = model.predict(
            source=image_numpy, conf=conf_threshold, device=device, imgsz=selected_size
        )
