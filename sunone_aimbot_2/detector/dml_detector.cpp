...
void filterDetectionsByDepthMask(
    ...
    std::vector<detection_debug::RawBoxDebug>*)
{
}
#endif
...
std::vector<std::vector<Detection>> DirectMLDetector::detectBatch(const std::vector<cv::Mat>& frames)
{
    ...
    for (size_t b = 0; b < batch_size; ++b)
    {
        ...
        detections = decodeYolo26Outputs(ptr, shp, num_classes, conf_thr, x_gain, y_gain, &rawBoxes);
    ...
    try
    {
        while (!shouldExit)
        {
            ...
            if (hasNewFrame && !frame.empty())
            {
                ...
                const std::vector<Detection>& filteredDetections = detectionsBatch.back();
