...
# Ensure local script imports work regardless of current working directory
...
def package_skill(skill_path, output_dir=None):
    ...
    # Avoid recursive self-inclusion when output_dir is inside skill_path
        ...
        with zipfile.ZipFile(skill_filename, "w", zipfile.ZIP_DEFLATED) as zipf:
            ...
            for file_path in skill_path.rglob("*"):
                if file_path.is_file():
                    relative_in_skill = file_path.relative_to(skill_path)

                    if any(
                        part in excluded_dir_names for part in relative_in_skill.parts
                    ):
                        continue

                    # Skip output directory contents when it lives under the skill folder
                    if output_path_resolved in file_path.resolve().parents:
                        continue

                    # Skip pre-existing packaged artifacts inside the skill tree
